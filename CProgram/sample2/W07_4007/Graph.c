...
void delete_node(GRAPH * graph, char node_label)
{
    ...
    for(i=0; i<graph->node_count; i++)
    {
        ...
        // TODO: retrieve removingIndex from the matching removing node.
    }
    ...
    if(removingIndex >= 0)
    {
        ...
        for(i=0; i<graph->node_count; i++)
        {
            for(j=removingIndex+1; j<graph->node_count; j++)
            {
                graph->adjacency[i][j];
            }
        }
        for(i=removingIndex+1; i<graph->node_count; i++)
        {
            ...
            // TODO: 3.1.1 Shift up all values under the removing row
            // TODO: 3.1.2 Shift left all values in the right of the removing column
